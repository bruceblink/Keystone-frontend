...
/**
 * 摄像机管理页面：船只选择、列表检索、增删改查、Excel 导入导出
 */
...
import { useCameraList } from "./utils";
import type { CameraItem } from "./utils/types";
import CameraDialog from "./components/CameraDialog.vue";
import { useBoatStoreHook } from "@/store/modules/boat";

defineOptions({ name: "ParamDeviceCamera" });

/** 全局船只 store：顶部选择器与列表 devid 联动 */
const boatStore = useBoatStoreHook();

/** 摄像机列表业务状态与方法（见 useCameraList） */
const {
  loading,
  searchQuery,
  dataList,
  ...
  getStatusText,
  getStatusTagType
} = useCameraList(toRef(boatStore, "selectedBoatId"));
...
<template>
  <div class="main">
    <!-- 船只选择：切换 devid 后由 useCameraList 监听并刷新列表 -->
    <div
    ...
    >
      ...
      <template v-slot="{ size, dynamicColumns }">
        <pure-table
          ...
          @selection-change="(rows: CameraItem[]) => (multipleSelection = rows)"
        >
          ...
          <template #inference="{ row }">
            {{ getInferenceText(row.inference) }}
          </template>
          <template #status="{ row }">
